...
const Base = {
  ...
  getDevConfig(baseConfig, cmdConfig, option) {
    const pages = Util.getPages(true, !!option.dll, option.entries);
  ...
  getBuildConfig(baseConfig, cmdConfig, option) {
    const pages = Util.getPages(false, false, option.entries);
    const webpackConfig = merge({}, baseConfig, {
      entry: pages.entries,
      output: {
        path: path.resolve(cwd, Config.get('dest')),
  ...
  getBuildLoadesAndPlugins(packOpt = {}, nominify) {
    ...
    NORMAL_LOADS.forEach((item, index) => {
